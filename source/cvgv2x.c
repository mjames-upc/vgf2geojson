 ...
 * M. James/Unidata	04/14	Modified from vgf2xml				* 
 ********************************************************************************/
{
int	ier;
/*---------------------------------------------------------------------*/
...
void	frt2tag ( VG_DBStruct *el, char *buffer, int *iret )
{
    ...
    sprintf( buffer, "              { \"type\": \"Feature\",\"geometry\": {\"type\": \"LineString\",\"coordinates\": [\n" );
    npts = el->elem.frt.info.numpts;
    if  ( npts > 0 )  {
	if (el->elem.frt.info.fpipdr >=0) {
	    for ( ii = 0; ii < npts; ii++ )  {
	    	blen = strlen(buffer);
	    	sprintf( &(buffer[blen]), "%s            [%10.6f, %10.6f],", indent, checkLongitude(el->elem.frt.latlon[ii+npts]), el->elem.frt.latlon[ii]);
	    }
	}
	else {
	...
	blen = strlen(buffer);
	sprintf( &(buffer[blen]), "%s            [%10.6f, %10.6f]", indent, 0.0, 0.0 );
...
void	lin2tag ( VG_DBStruct *el, char *buffer, int *iret )
{
    ...
    npts = el->elem.frt.info.numpts;
    if  ( npts > 0 )  {
        sprintf( buffer, "   { 'type': 'Feature', \n     'geometry': { \n       'type': 'LineString', \n       'coordinates': [ \n" );
        for ( ii = 0; ii < npts; ii++ )  {
            blen = strlen(buffer);
            sprintf( &(buffer[blen]), "[%10.6f, %10.6f]", checkLongitude(el->elem.lin.latlon[ii+npts]), el->elem.lin.latlon[ii]);
	    if ( ii < npts - 1 )  {
                blen = strlen(buffer);
	        sprintf( &(buffer[blen]),",");
	    }
...
*/
    ...
    if  ( npts > 0 )  {
        sprintf( buffer, "   { 'type': 'Feature', 'geometry': {'type': 'Point', 'coordinates': " );
	for ( ii = 0; ii < npts; ii++ )  {
	    blen = strlen(buffer);
            sprintf( &(buffer[blen]), "[%10.6f, %10.6f]", checkLongitude(el->elem.sym.data.latlon[ii+npts]), el->elem.sym.data.latlon[ii]);
	}
        blen = strlen(buffer);
        sprintf( &(buffer[blen]), "}, 'properties': {'name': '%s'} },\n", pgenType );
    }
...
// points, text and ending tag
    ...
    blen = strlen(buffer);
    sprintf( &(buffer[blen]), "[%10.6f, %10.6f]", checkLongitude(el->elem.spt.info.lon), el->elem.spt.info.lat );
	...
	if (el->hdr.grptyp ==1 ) {
		...
		getEscape(result, escape); 
    	        sprintf( &(buffer[blen]), "}, 'properties': {'name': '%s', 'yoffset': '%d', 'xoffset': '%d'} },\n", escape, el->elem.spt.info.offset_y, el->elem.spt.info.offset_x );
                result = strtok( NULL, "\r\n" ); 
	    }
	    free(escape);
	}
...
//sub2 -- barb
	...
	// PRINT TEXT OBJECTS TO GEOJSON
	blen = strlen(buffer);
        sprintf( &(buffer[blen]), "   { 'type': 'Feature', \n    'geometry': {'type': 'Point', 'coordinates': " );
    	/*
...
mask, displayType, el->elem.jet.barb[ii].spt.info.offset_y, el->elem.jet.barb[ii].spt.info.offset_x, "false", "false");
	*/
	...
	lat = el->elem.jet.barb[ii].spt.info.lat + (el->elem.jet.barb[ii].spt.info.offset_y/2) *cos(radians) + (el->elem.jet.barb[ii].spt.info.offset_x/2) *cos(radians); 
    	...
    	blen = strlen(buffer);
        sprintf( &(buffer[blen]), "[%10.6f, %10.6f]", lon, lat );
    	result = strtok( el->elem.jet.barb[ii].spt.text, "\n" );
   	blen = strlen(buffer);
        sprintf( &(buffer[blen]), "}, 'properties': {'name': '%s', 'yoffset': '%d', 'xoffset': '%d'} },\n", result, el->elem.jet.barb[ii].spt.info.offset_y, el->elem.jet.barb[ii].spt.info.offset_x );
